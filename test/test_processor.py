...
class TestSQSProcessor:
    ...
    async def test_get_queue_url_existing_queue(self):
        ...
        assert self.processor.queue_url == "http://localhost:4566/000000000000/test-queue"
    ...
    async def test_get_queue_url_create_new_queue(self):
        ...
        self.processor.sqs_client = Mock()
        self.processor.sqs_client.get_queue_url.side_effect = Exception("Queue not found")
        ...
        assert self.processor.queue_url == "http://localhost:4566/000000000000/new-queue"
    ...
    @patch('src.processor.main.time.sleep')
    @patch('src.processor.main.logger')
    def test_wait_for_redis_retry_then_success(self, mock_logger, mock_sleep, mock_redis_client):
    ...
    @patch('src.processor.main.time.sleep')
    @patch('src.processor.main.logger')
    def test_wait_for_redis_max_retries_exceeded(self, mock_logger, mock_sleep, mock_redis_client):
    ...
    async def test_run_redis_connection_failure(self, mock_exit, mock_sleep, mock_redis_client):
    ...
    async def test_run_successful_processing(self, mock_sleep, mock_redis_client):
        ...
        with patch.object(self.processor, '_wait_for_redis', return_value=True), \
             patch.object(self.processor, '_get_queue_url', return_value="test-queue"), \
             patch.object(self.processor, 'process_messages') as mock_process:
            ...
            def side_effect():
                ...
                if call_count > 4:
                    self.processor.running = False
                return mock_process.side_effect[call_count - 1] if call_count <= 4 else 0
    ...
    @patch('src.processor.main.time.sleep')
    @patch('src.processor.main.logger')
    async def test_run_processing_loop_error_handling(self, mock_logger, mock_sleep, mock_redis_client):
        ...
        with patch.object(self.processor, '_wait_for_redis', return_value=True), \
            ...
            def side_effect():
                ...
                if call_count > 2:
                    self.processor.running = False
                return original_side_effect[call_count - 1] if call_count <= len(original_side_effect) else 0
...
class TestMainFunction:
    ...
    @patch('src.processor.main.SQSProcessor')
    @patch('src.processor.main.logger')
    def test_main_function_keyboard_interrupt(self, mock_logger, mock_processor_class, mock_asyncio_run):
    ...
    @patch('src.processor.main.logger')
    @patch('src.processor.main.sys.exit')
    def test_main_function_exception(self, mock_exit, mock_logger, mock_processor_class, mock_asyncio_run):
