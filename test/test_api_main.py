...
class TestAPIEndpoints:
    ...
    def test_get_all_stats_success(self, test_client, mock_stats_service):
        ...
        mock_stats = [
            StatsResponse(event_type="user_signup", count=5.0, total=250.0, average=50.0),
            StatsResponse(event_type="user_login", count=10.0, total=150.0, average=15.0)
        ]
    ...
    def test_get_stats_by_type_special_characters(self, test_client, mock_stats_service):
    ...
    @patch('src.api.main.logger')
    def test_get_stats_by_type_error(self, mock_logger, test_client, mock_stats_service):
...
class TestAPIMetadata:
    ...
    def test_app_metadata(self, test_client):
        ...
        assert app.description == "FastAPI application for retrieving SQS message processing statistics"
...
class TestAPILifespan:
    ...
    @patch('src.api.main.stats_service')
    @patch('src.api.main.logger')
    def test_lifespan_redis_connection_retry(self, mock_logger, mock_stats_service, mock_sleep):
...
class TestAPIIntegration:
    ...
    def test_full_api_workflow(self, test_client, mock_stats_service):
        ...
        mock_all_stats = [
            StatsResponse(event_type="user_signup", count=5.0, total=100.0, average=20.0),
