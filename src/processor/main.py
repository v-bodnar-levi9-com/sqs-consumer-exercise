    ...
    async def _get_queue_url(self):
        ...
        if self.queue_url is None:
            try:
                ...
                logger.info(f"Queue {Config.SQS_QUEUE_NAME} not found, creating new queue")
    ...
    def process_messages(self):
        ...
        try:
            ...
            for message in messages:
                ...
                try:
                    # Update Redis stats
                    redis_client.increment_event(message_data.type, float(message_data.value))
                    
                    # Delete message from SQS
                    self.sqs_client.delete_message(QueueUrl=queue_url, ReceiptHandle=receipt_handle)
                    
                    processed_count += 1
                    logger.debug(f"Processed message: type={message_data.type}, value={message_data.value}")
